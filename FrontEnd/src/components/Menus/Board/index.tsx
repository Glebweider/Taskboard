    ...
    return (
        <>
            <div
                style={styles}
                ref={menuRef}
                className={`${style.container} ${isVisible ? style.open : ''}`}>
                <div
            ...
            {createPortal(<CreateModal
                ...
                onSubmit={async (name) => {
                    ...
                    setProjects(prev =>
                        prev.map(proj => ({
                            ...proj,
                            boards: proj.boards.map((board: IBoardPreviewCard) =>
                                board.id === data.id
            ...
            {createPortal(<DeleteModal
                isOpen={menuState == EMenuState.Delete}
                type={ETypeDelete.BOARD}
                title={board.name}
                countClicks={0}
                onClose={() => setMenuState(EMenuState.None)}
                onSubmit={async () => {
                    ...
                    setProjects(prev =>
                        prev.map(p =>
                            p.id === project.id ?
                                { ...p, boards: p.boards.filter(b => b.id !== board.id) } : p
                        )
                    );
                }} />,
                document.body
